...
public class FrameNuevaConsulta implements DocumentListener, ActionListener {
	...
	private void initalizeFrame(JFrame framePadre) {
		...
		*/nuevaConsultaPanel.add(new JScrollPane(this.tablaMascotas), constraints);
	...
	private JTable cargarTablaMascotas() {
		...
		/*
		 * El tama�o de la tabla es, 4 columnas (cantidad de datos a mostrar) y
	...
	private void accionIngesar() {
		...
		if (fecha == null || fecha.after(new Date())) {
			JOptionPane.showMessageDialog(frame, "Debe seleccionar una fecha Valida para la consulta", "Datos incompletos!",
					JOptionPane.WARNING_MESSAGE);
		...
		if (almacenado) {
			JOptionPane.showMessageDialog(frame, "La consulta se ha registrado con �xito.", "Consulta Registrada!",
